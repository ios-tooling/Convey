...
@ConveyActor open class ConveyServer: ObservableObject {
	...
	open func preflight(_ task: ServerConveyable, request: URLRequest) async throws -> URLRequest {
	...
	open func postflight(_ task: ServerConveyable, result: ServerResponse) { }
	
	open func taskFailed(_ task: ServerConveyable, error: Error) async { print("Error: \(error) from \(task)") }
	
	open func standardHeaders(for task: ServerConveyable) async throws -> [String: String] {
	...
	open func url(forTask task: ServerConveyable) -> URL {
		var path = task.path
		if path.hasPrefix("/") { path.removeFirst() }
		return baseURL.appendingPathComponent(path)
	}
	
	open var reportConnectionError: (ServerConveyable, Int, String?) -> Void = { task, code, description in
