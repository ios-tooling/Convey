...
public extension ServerConveyable {
	...
	func defaultRequest() async throws -> URLRequest {
		...
		var isGzipped = self is any GZipEncodedUploadingTask
		
		request.timeoutInterval = await timeout
		request.httpMethod = httpMethod
		request.cachePolicy = .reloadIgnoringLocalCacheData
		if let dataProvider = self.wrappedTask as? any DataUploadingTask {
		...
		if let tagged = self.wrappedTask as? any TaggedTask {
		...
		if self.wrappedTask is any ETagCachedTask, let etag = cachedEtag, DataCache.instance.hasCachedValue(for: url) {
	...
	var httpMethod: String {
		if self.wrappedTask is any ServerPOSTTask { return "POST" }
		if self.wrappedTask is any ServerPUTTask { return "PUT" }
		if self.wrappedTask is any ServerPATCHTask { return "PATCH" }
		if self.wrappedTask is any ServerDELETETask { return "DELETE" }
		return "GET"
	}
}
