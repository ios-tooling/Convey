...
//		}
		...
		do {
			...
			if error.isOffline, self.wrappedTask is any FileBackedTask, let cache = DataCache.instance.fetchLocal(for: url) {
				let response = ServerResponse(response: HTTPURLResponse(cachedFor: url, data: cache.data), data: cache.data, fromCache: true, duration: 0, startedAt: cache.cachedAt, retryCount: nil)
				await willComplete(with: response)
				await didComplete(with: response)
				return response
...
//			}
			...
			while true {
				do {
					...
					(self.wrappedTask as? any ArchivingTask)?.archive(result)
					...
					if result.statusCode == 304, let data = DataCache.instance.fetchLocal(for: url), !data.data.isEmpty {
						...
						if self.wrappedTask is any FileBackedTask { self.fileCachedData = result.data }
					}
					
					if self.wrappedTask is any ETagCachedTask, let tag = result.response.etag {
					...
					if !result.response.didDownloadSuccessfully {
						...
						if result.data.isEmpty || (result.statusCode.isHTTPError && reportBadHTTPStatusAsError) {
