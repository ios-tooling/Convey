...
@available(iOS 17, macOS 14, watchOS 10, *)
struct ObserveTaskAndPayloadModifier<Target: DownloadingTask>: ViewModifier {
	@State var token = ""
	var closure: @MainActor @Sendable (Target, Target.DownloadPayload?, Error?) -> Void
...
public extension View {
	...
	@ViewBuilder func observeTask<Target: DownloadingTask>(_ type: Target.Type, callback: @MainActor @escaping (Target, Target.DownloadPayload?, Error?) -> Void) -> some View {
		self
			.modifier(ObserveTaskAndPayloadModifier(closure: callback))
	}
}
