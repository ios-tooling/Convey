...
- **Test harness app**: `Test Harness/ConveyTestHarness/` - SwiftUI test application (open `Test Harness/ConveyTestHarness.xcodeproj` in Xcode and run the ConveyTestHarness target)
...
- **Dependencies**: JohnnyCache (https://github.com/ios-tooling/JohnnyCache.git) for caching, Chronicle (https://github.com/ios-tooling/Chronicle/) for logging, and TagAlong (https://github.com/ios-tooling/TagAlong/) for task tags and configuration helpers
...
- **Default server pattern**: Tasks without an explicit `server` property use `ConveyServer.default`. Assign the application server to that static property during setup.
...
### Platform Support
- Package-declared minimum versions: iOS 14+, macOS 14+, watchOS 10+
...
- **ConveySession**: Manages individual network sessions and URLSession instances. Servers can cancel active work by request ID or task type.
...
- Use the `-fail-all-requests` launch argument in DEBUG builds to fail requests immediately
